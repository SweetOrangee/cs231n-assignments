...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_cur = h0
    for t in range(T):
      h_cur, cache_cur = rnn_step_forward(x[:, t, :], h_cur, Wx, Wh, b)
      h.append(h_cur)
...
def rnn_backward(dh, cache):
    ...
    N, D = cache[0][0].shape
    dx = np.zeros((N, T, D))
    dh0 = np.zeros_like(cache[0][1])
    dWx = np.zeros_like(cache[0][2])
    dWh = np.zeros_like(cache[0][3])
    db = np.zeros_like(cache[0][4])

    for t in reversed(range(T)):
      dx_cur, dh0, dWx_cur, dWh_cur, db_cur = rnn_step_backward(dh[:, t, :] + dh0, cache[t])
      dx[:, t, :] = dx_cur
      dWx += dWx_cur
      dWh += dWh_cur
      db += db_cur
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    out = (i, f, o, g, next_c)
    cache = (x, prev_h, prev_c, Wx, Wh, b, out)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, out = cache
    i, f, o, g, next_c = out
    N, H = prev_h.shape
    da = np.zeros((N, 4 * H))
    # dnext_h  [N, H]
    ...
    dnext_c = dnext_c + dnext_h * o * (1 - tanh_next_c * tanh_next_c)
    ...
    dx = da.dot(Wx.T) # W[D,4H]
    dprev_h = da.dot(Wh.T)
    dWx = x.T.dot(da)# x[N, D]
    dWh = prev_h.T.dot(da)
    db = np.sum(da, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    _, T, _ = x.shape
    h_cur = h0
    c_cur = np.zeros_like(h0)
    for t in range(T):
      h_cur, c_cur, cache_cur = lstm_step_forward(x[:, t, :], h_cur, c_cur, Wx, Wh, b)
      h.append(h_cur)
      cache.append(cache_cur)
    h = np.stack(h, axis=1)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dx_cur, dh_cur, dc, dWx_cur, dWh_cur, db_cur = lstm_step_backward(dh[:, t, :] + dh_cur, dc, cache[t])
      dx[:, t, :] = dx_cur
      dWx += dWx_cur
      dWh += dWh_cur
      db += db_cur
