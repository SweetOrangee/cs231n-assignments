...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        word_vec = self.embedding(captions)
        word_vec = self.positional_encoding(word_vec)
        features_projected = self.visual_projection(features).unsqueeze(dim=1)
        tgt_mask = torch.ones((T, T), dtype=bool)
        tgt_mask = torch.tril(tgt_mask)
        output = self.transformer(word_vec, features_projected, tgt_mask)
