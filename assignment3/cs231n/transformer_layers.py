...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, ::2] = torch.tensor([[math.sin(i / (10000 ** (j / embed_dim)))
          for j in range(0, embed_dim, 2)] for i in range(max_len)] )
        pe[0, :, 1::2] = torch.tensor([[math.cos(i / (10000 ** (j / embed_dim)))
          for j in range(0, embed_dim - 1, 2)] for i in range(max_len)])
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = q.matmul(k) / math.sqrt(self.head_dim)
        if attn_mask != None:
          scores = torch.masked_fill(scores, ~attn_mask, -math.inf)
        weights = F.softmax(scores, dim=3)
        
        y = self.attn_drop(weights)
        y = y.matmul(v)
        y = y.permute(0, 2, 1, 3).reshape(N, S, E)
        output = self.proj(y)
