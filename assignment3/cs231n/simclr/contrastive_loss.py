...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j) / (z_i.norm() * z_j.norm())
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k_kN, l_kN_k = 0, 0
        for i in range(N):
          l_k_kN += torch.exp(sim(z_k, out[i]) / tau) + torch.exp(sim(z_k, out[i + N]) / tau)
          l_kN_k += torch.exp(sim(z_k_N, out[i]) / tau) + torch.exp(sim(z_k_N, out[i + N]) / tau)
        l_k_kN = -sim(z_k, z_k_N) / tau + torch.log(l_k_kN - np.exp(1 / tau))
        l_kN_k = -sim(z_k, z_k_N) / tau + torch.log(l_kN_k - np.exp(1 / tau))
        total_loss = total_loss + l_k_kN + l_kN_k
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(dim=1) / (torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1))
...
def compute_sim_matrix(out):
    ...
    sim_matrix /= norm
    sim_matrix /= norm.unsqueeze(dim=1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = (pos_pairs / tau).repeat(2, 1).squeeze()
    ...
    loss = torch.sum(-numerator + torch.log(denom)) / (2 * N)
